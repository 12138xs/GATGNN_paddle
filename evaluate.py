...
true_label, pred_label = paddle.to_tensor([]),paddle.to_tensor([])
testset_idx    = paddle.to_tensor([])
num_elements   = paddle.to_tensor([])
net.eval()
...
for data in test_loader:
    ...
    true_label      = paddle.concat([true_label, data.y],axis=0)
    pred_label      = paddle.concat([pred_label, predictions],axis=0)
    testset_idx     = paddle.concat([testset_idx, data.the_idx],axis=0)
    num_elements    = paddle.concat([num_elements, data.num_atoms],axis=0)


test_result    = metrics.eval_func(pred_label,true_label).item()
